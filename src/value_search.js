...
function processQueryResults(allFeatures, key, value) {
    ...
    featuresWithTags.forEach((feature, index) => {
        ...
        if (modelFilename) {
            ...
            const modelUrl = `${window.location.origin}/3dmodelsosm/src/models/${modelFilename}`;
            const modelOptions = {
                uri: modelUrl,
...
/**
 * Display value search results to the user
 * @param {Array} results - Array of search result objects
 * @param {string} query - The original search query
 */
function displayValueResults(results, query) {
    console.log(`🔍 Displaying ${results.length} value search results for query: "${query}"`);
    
    // For now, just log the results - could be enhanced to show in UI
    results.forEach((result, index) => {
        console.log(`  ${index + 1}. ${result.key}=${result.value} (${result.count} uses)`);
        if (result.description) {
            console.log(`     Description: ${result.description}`);
        }
    });
    
    // TODO: Implement actual UI display for search results
    // This could show results in a popup, sidebar, or dedicated results panel
}
/**
...
function executeTagQuery(key, value) {
    ...
    console.log('🚀 Event detail:', { key, value, overlayId: `tag_${key}_${value}` });
    window.dispatchEvent(new CustomEvent('tagQueryAdded', {
